...
class SpatialManager {
  /// Internal cache for facility layouts
  final Map<String, List<WorkstationConfig>> _layoutCache = {};
  ...
  List<WorkstationConfig> getLayoutForFacility(String facilityId) {
    if (_layoutCache.containsKey(facilityId)) {
      return _layoutCache[facilityId]!;
    }

    final List<WorkstationConfig> allConfigs = _generateAllConfigurations();
    List<WorkstationConfig> result;

    if (facilityId == 'CT1') {
      result = allConfigs;
    } else if (facilityId.startsWith('Lab ') || facilityId.startsWith('Laboratory ')) {
      final labNumber = facilityId.split(' ').last;
      final labPrefix = 'L${labNumber}_';
      result = allConfigs.where((config) => config.id.startsWith(labPrefix)).toList();
    } else {
      result = [];
    }

    _layoutCache[facilityId] = result;
    return result;
  }
