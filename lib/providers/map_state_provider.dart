...
/// Optimized lookup map for the Map Canvas painter.
/// Maps desk IDs to the list of assets present at that desk.
final inventoryLookupMapProvider = Provider<Map<String, List<Map<String, dynamic>>>>((ref) {
  final inventoryAsync = ref.watch(facilityInventoryProvider);
  return inventoryAsync.when(
    data: (inventory) {
      final Map<String, List<Map<String, dynamic>>> lookup = {};
      for (final asset in inventory) {
        final deskId = asset['location']?['name'] as String?;
        if (deskId != null) {
          lookup.putIfAbsent(deskId, () => []).add(asset);
        }
      }
      return lookup;
    },
...
final facilityInventoryProvider = StreamProvider<List<Map<String, dynamic>>>((ref) {
  ...
  Future<void> fetchAndEmit() async {
    try {
      ...
      // Server-side filtering for specific labs
      if (selectedFacility.startsWith('Lab ')) {
        final labNumber = selectedFacility.split(' ').last;
        query = query.filter('locations.name', 'ilike', 'L${labNumber}_%');
      }
          
      final response = await query;
          
      if (!controller.isClosed) {
        final filtered = (response as List<dynamic>).cast<Map<String, dynamic>>().toList();
